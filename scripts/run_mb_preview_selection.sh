...
for iteration in $PREVIEW_ITERATIONS; do
  ...
  candidate_args+=(--candidate "${id}=${summary},${checkpoint}")
done
...
if [[ -n "$FALLBACK_PREVIEW_CHECKPOINT" ]]; then
  ...
  candidate_args+=(--candidate "${FALLBACK_PREVIEW_ID}=${summary},${FALLBACK_PREVIEW_CHECKPOINT}")
...
for mode in random battle-model; do
  ...
  baseline_args+=(--baseline "${id}=${summary}")
done
