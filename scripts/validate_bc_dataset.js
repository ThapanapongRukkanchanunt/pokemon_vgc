...
function validateExample(example, fileName, lineNumber, errors) {
  ...
  if (typeof example.label_action_index !== 'number') {
    errors.push(`${prefix} missing numeric label_action_index`);
  } else if (
    Array.isArray(example.legal_actions) &&
    example.legal_actions[example.label_action_index] !== example.label_action
  ) {
    errors.push(`${prefix} label_action_index does not point to label_action`);
  }
  if (![null, 'p1', 'p2'].includes(example.winner_side)) {
    errors.push(`${prefix} invalid winner_side`);
  }
  if (![null, 0, 1].includes(example.win_target)) {
    errors.push(`${prefix} invalid win_target`);
  }
...
function main() {
  try {
    ...
    console.log(`Winners: ${JSON.stringify(counts.winners)}`);
    console.log(`Teams: ${JSON.stringify(counts.teams)}`);
